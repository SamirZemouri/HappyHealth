...
function get_health_champion($conn){
    // Requête SQL pour récupérer les colonnes "country", "year" et "health score"
    $req_select_health_champion = "SELECT country, `year`, MAX(`value`) FROM countries_has_years INNER JOIN countries ON countries_has_years.countries_id = countries.id INNER JOIN years ON countries_has_years.years_id = years.id INNER JOIN all_values ON countries_has_years.values_health_score_id = all_values.id WHERE `year` = :year GROUP BY country ORDER BY `value` DESC LIMIT 1";
    // Préparation de la requête
    $req_health_champion = $conn->prepare($req_select_health_champion);
